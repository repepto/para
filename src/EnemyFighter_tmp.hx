...
class EnemyFighter extends Enemy
{
	...
	public function new(body:Body, life:Int, velocity:Int)
	{
		...
		else if (Game.game.currentLevel >= 8) 
		{
			velocity += 0;
			angDiv = 10;
		}
	...
	override function controll() 
	{
		...
		var step = (targetAng - body.rotation) / angDiv;
	...
	override function fire()
	{
		...
		if (Game.game.currentLevel >= 5 && Game.game.currentLevel < 8) randFire = 37;
		else if (Game.game.currentLevel >= 8) randFire = 20;
		
		fireDelay = randFire + Std.random(randFire);
		...
		if (Game.game.currentLevel >= 5 && Game.game.currentLevel < 8) vel += 170;
		else if (Game.game.currentLevel >= 8) vel +=470;
